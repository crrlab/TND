...
{	struct agent__setCameraURI soap_tmp_agent__setCameraURI;
	...
	soap_default_camera__info(soap, &_infoResponse);
	...
	if (soap_body_end_in(soap)
	 ...
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = soap->setCameraURI(soap_tmp_agent__setCameraURI._info, _infoResponse);
	if (soap->error)
		return soap->error;
	soap_serializeheader(soap);
	soap_serialize_camera__info(soap, &_infoResponse);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || soap_put_camera__info(soap, &_infoResponse, "camera:info", NULL)
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};
