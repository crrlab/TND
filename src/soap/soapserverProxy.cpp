...
int serverProxy::setCameraURI(struct camera__uri *_info, std::string &message)
{	struct soap *soap = this;
	...
	const char *soap_action = NULL;
	soap->encodingStyle = NULL;
	soap_tmp_server__setCameraURI._info = _info;
	soap_begin(soap);
	soap_serializeheader(soap);
	soap_serialize_server__setCameraURI(soap, &soap_tmp_server__setCameraURI);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || soap_put_server__setCameraURI(soap, &soap_tmp_server__setCameraURI, "server:setCameraURI", NULL)
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	}
	...
	if (soap_begin_recv(soap)
	 ...
	 || soap_body_begin_in(soap))
		return soap_closesock(soap);
	soap_tmp_server__setCameraURIResponse = soap_get_server__setCameraURIResponse(soap, NULL, "server:setCameraURIResponse", "");
	if (soap->error)
		return soap_recv_fault(soap, 0);
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap_closesock(soap);
	message = soap_tmp_server__setCameraURIResponse->message;
	return soap_closesock(soap);
}
