...
void ManagerProxy::Update(Pattern::Subject* theChangeSubject) {
	...
	if (rvt->getInfo("command") == "removeAll") {
		...
		if (dynamic_cast<SMC::StorageProxy*>(theChangeSubject)) {
			...
			std::clog << "[SMC::ManagerProxy] Agent: "
					<< this->Endpoints["Soap_Service"] << std::endl;
			mapStreamingProfile::iterator it;
			for (it = nvt->FileSource.begin(); it != nvt->FileSource.end();
					++it) {
	...
	if (dynamic_cast<SMC::FrontendProxy*>(theChangeSubject)) {
			...
			if (nvt->getInfo("command") == "diretta"
				...
				this->SetOptionCall("SetCameraInfo", "info_type", "switch");
				this->SetOptionCall("SetCameraInfo", "info",
...
/*
			if (nvt->getInfo("command") == "live"
					&& (!nvt->getInfo("live").empty())) {
				nvt->SetInfo("command", "");

				std::clog << "[SMC::ManagerProxy]: live channel: "
						<< nvt->getInfo("live") << std::endl;
				this->SetOptionCall("SetCameraInfo", "uuid",
						nvt->getInfo("live"));
				this->SetOptionCall("SetCameraInfo", "info_type", "live");
				this->SetOptionCall("SetCameraInfo", "info",
						nvt->getInfo("live"));
				this->Call("SetCameraInfo");
			...
			if (nvt->getInfo("command") == "differita"
					&& (!nvt->getInfo("differita").empty())) {
				nvt->SetInfo("command", "");

				std::clog << "[SMC::ManagerProxy]: differita channel: "
						<< nvt->getInfo("differita") << std::endl;
				this->SetOptionCall("SetCameraInfo", "uuid",
						nvt->getInfo("differita"));
				this->SetOptionCall("SetCameraInfo", "info_type", "differita");
				this->SetOptionCall("SetCameraInfo", "info",
						nvt->getInfo("differita"));
				this->Call("SetCameraInfo");
